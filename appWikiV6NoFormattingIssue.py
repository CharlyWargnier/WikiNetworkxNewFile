...
with c32:
  st.header('')
  st.header('')
  st.markdown('###### Original script by JR Oakes - Ported to [![this is an image link](https://i.imgur.com/iIOA6kU.png)](https://www.streamlit.io/)&nbsp, with :heart: by [@DataChaz](https://twitter.com/DataChaz) &nbsp [![this is an image link](https://i.imgur.com/thJhzOO.png)](https://www.buymeacoffee.com/cwar05)')
...
-   Disable service account when finished      
...
st.markdown('## **① Upload your Google NLP credentials **') #########
#st.text('')
...
def google_nlp_entities(input, input_type='html', result_type="all", limit=10, invalid_types = ['OTHER', 'NUMBER', 'DATE']):
...
def load_text_from_url(url, **data):
    ...
    try:
        ...
        response = requests.get(url,headers = {'user-agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:68.0) Gecko/20100101 Firefox/68.0'}, timeout=timeout)
...
def get_wikipedia_url(query):
    ...
    search = sitew.search(query, where='title', get_redirects=True, total=1, content=False, namespaces="0")
    for page in search:
...
@st.cache(allow_output_mutation=True,show_spinner=False)
def recurse_entities(input_data, entity_results=[], G=nx.Graph(), current_depth=0, depth=2, limit=3):
    '''
    ...
    if isinstance(input_data,str):
        ...
        input_data = [{'name': name.title(),'type': 'START','salience': 0.0,'wikipedia': input_data}] if input_data else []       
    ...
    for d in input_data:
        ...
        print('   '*current_depth + 'Level: {0} Name: {1}'.format(current_depth, name))
        ...
        results= [r for r in google_nlp_entities(html, input_type='text', limit=None, result_type="wikipedia") if 'wiki' not in r['name'].lower() and not G.has_node(r['name'])][:limit]
...
def plot_entity_branches(G, w=10, h=10, c=1, font_size=14, filename=None):
    ...
    new_pos = {u:(r*math.sin(theta),r*math.cos(theta)) for u, (theta, r) in pos.items()}
    nx.draw(G, pos=new_pos, alpha=0.8, node_size = 25, with_labels=True, font_size=font_size, edge_color='gray')
    nx.draw_networkx_nodes(G, pos=new_pos, nodelist = [start], node_color = 'blue', node_size = 500)
...
c10, c0, c8, c1, c2, c3, c4, c5, c6 = st.beta_columns([0.10,0.50,0.10,8,0.10,1.5,0.10,1.5,0.10])
        ...
        if toggle == 'KW':
            ...
            if keyword:
                if search(substring, keyword):
                    st.warning('⚠️ Seems like you&#39re trying to paste a URL, switch to &#39URL&#39 mode!')
                else:
            ...
            keyword = st.text_input('Enter a Wikipedia URL', 'https://en.wikipedia.org/wiki/Diet_(nutrition)', key = 2)
...
button1 = c20.button('Happy with costs, get me my data!')
...
WikiNetworkx = hv.Graph.from_networkx(fig, nx.layout.circular_layout).opts(tools=['hover'])
